...
/* LAYOUT CLÁSSICO android_native_app_glue (ELF32 ARM) — offsets CONFIRMADOS
 * via disasm do glue estático do libShantaeCurse_android.so (WayForward "Black"
 * engine). sizeof = 148 (0x94). Campos pthread (mutex@64/cond@68) e thread@80
 * são slots de 4 bytes (bionic arm32) tratados pelo pthread_bridge como ponteiro
 * p/ o objeto glibc real. NÃO embutir pthread_mutex_t/cond_t do glibc (24/48B)
 * aqui — quebraria os offsets. */
typedef struct ShimARect { int32_t left, top, right, bottom; } ShimARect;
struct android_app {
  void *userData;                                       /* 0   */
  void (*onAppCmd)(struct android_app *app, int32_t cmd);/* 4  */
  int32_t (*onInputEvent)(struct android_app *app, AInputEvent *event); /* 8 */
  ...
  ShimARect contentRect;                                /* 40..55 */
  int activityState;                                    /* 56  */
  int destroyRequested;                                 /* 60  */
  void *_mutex;                                         /* 64 (bridge: ptr glibc) */
  void *_cond;                                          /* 68 (bridge: ptr glibc) */
  int msgread;                                          /* 72  */
  int msgwrite;                                         /* 76  */
  void *_thread;                                        /* 80 (pthread_t slot, 4B) */
  struct android_poll_source cmdPollSource;             /* 84..95 */
  struct android_poll_source inputPollSource;           /* 96..107 */
  int running;                                          /* 108 */
  int stateSaved;                                       /* 112 */
  int destroyed;                                        /* 116 */
  ...
  ShimARect pendingContentRect;                         /* 132..147 */
};
