...
int so_load(const char *filename, void *base, size_t max_size) {
  ...
  for (int i = 0; i < elf_hdr->e_phnum; i++) {
    if (prog_hdr[i].p_type == PT_LOAD) {
      ...
      /* NDK/lld moderno gera VÁRIOS PT_LOAD, com 2+ executáveis (o grande de
         código + um pequeno de trampolins/plt). Escolher o PRIMEIRO PF_X como
         texto (o de código, vaddr 0); pegar o ÚLTIMO dava o segmento errado
         (text_base off 0x149c000 -> relocations liam lixo, "unknown reloc"). */
      if ((prog_hdr[i].p_flags & PF_X) == PF_X) {
        if (text_segno < 0) text_segno = i;
      } else {
        if (data_segno < 0) data_segno = i;
      }
      /* load_size = fim máximo entre TODOS os segmentos (cobre os 6 PT_LOAD) */
      size_t end = prog_hdr[i].p_vaddr + prog_size;
  ...
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
    ...
    if (strcmp(sh_name, ".rela.dyn") == 0 ||
      ...
      for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
        ...
        switch (type) {
        case 0: /* R_AARCH64_NONE — padding na tabela RELA, pular */
          break;
...
int so_patch_lazy_plt(uintptr_t handler, uintptr_t *out_lazy_val,
  ...
  uintptr_t plt_link = 0, got_run = 0; size_t got_n = 0;
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
    char *n = shstrtab + sec_hdr[i].sh_name;
    if (strcmp(n, ".plt") == 0)
      plt_link = sec_hdr[i].sh_addr;               /* valor-lazy cru = addr link-time da .plt (PLT0) */
    else if (strcmp(n, ".got") == 0) {
      got_run = (uintptr_t)text_base + sec_hdr[i].sh_addr;
      got_n = sec_hdr[i].sh_size / sizeof(uintptr_t);
    }
  }
  if (out_lazy_val) *out_lazy_val = plt_link;
  if (out_got_base) *out_got_base = got_run;
  if (!plt_link || !got_run) return -1;
  int c = 0;
  uintptr_t *g = (uintptr_t *)got_run;
  for (size_t i = 0; i < got_n; i++) {
    if (g[i] == plt_link) { if (handler) g[i] = handler; c++; }
  }
  return c;
}
  ...
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
    ...
    if (strcmp(sh_name, ".init_array") == 0) {
      ...
      for (int j = 0; j < cnt; j++) {
        ...
        /* ELD_INITGUARD: pula init que crasha (lazy PLT pairip não resolvido) e segue. */
        if (getenv("ELD_INITGUARD")) {
          extern sigjmp_buf g_eld_init_jmp; extern volatile int g_eld_init_armed;
          if (sigsetjmp(g_eld_init_jmp, 1)) {
            extern volatile unsigned long g_eld_init_crash_pc, g_eld_init_crash_lr, g_eld_init_crash_fault;
            debugPrintf("init_array[%d] CRASHOU @pc=0x%lx lr=0x%lx fault=0x%lx -> pulado\n",
                        j, g_eld_init_crash_pc, g_eld_init_crash_lr, g_eld_init_crash_fault);
            g_eld_init_armed = 0;
            continue;
          }
          g_eld_init_armed = 1;
...
/* Libera o so_base (CÓPIA do arquivo .so cru em malloc, p/ libil2cpp = 70MB, libunity
 * = 21MB) de um módulo JÁ totalmente carregado (relocate+resolve+record_phdr+init_array
 * feitos). so_base só é usado em tempo de LOAD (sec_hdr/shstrtab/elf_hdr/prog_hdr); em
 * runtime os símbolos/relocs já estão na imagem (text_base) e o phdr foi copiado p/
 * g_so_mods. Liberar aqui corta ~91MB de RSS = elimina o swap = boot determinístico.
 * Zera o ponteiro no módulo E o global (se casar) p/ evitar double-free no so_unload. */
void so_free_module_temp(so_module *m) {
  if (!m || !m->so_base) return;
  void *p = m->so_base;
  m->so_base = NULL;
  if (so_base == p) so_base = NULL;   /* módulo ativo: limpa o espelho global */
  free(p);
}
