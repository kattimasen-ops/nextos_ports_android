...
static int sh_sem_wait(void *s) {
  ...
  if(getenv("DUCK_SEMSTUCK")){
    struct timespec ts; int waited=0; void *caller=__builtin_return_address(0);
    for(;;){ clock_gettime(CLOCK_REALTIME,&ts); ts.tv_sec+=1;
      int r=sem_timedwait(g,&ts);
      if(r==0) return 0;
      if(errno==ETIMEDOUT){ waited++;
        if(waited==2||waited==6){ int v=-1; sem_getvalue(g,&v);
          fprintf(stderr,"[SEMSTUCK] wait b=%p glibc=%p val=%d waited=%ds tid=%p caller=%p\n",
            s,(void*)g,v,waited,(void*)pthread_self(),caller); }
        continue; }
      return r; }
  }
  return sem_wait(g); }
...
static int sh_sem_post(void *s) {
  if(getenv("DUCK_SEMSTUCK")){static int n=0;if(n++<100000)fprintf(stderr,"[SEMPOST] b=%p tid=%p caller=%p\n",s,(void*)pthread_self(),__builtin_return_address(0));}
...
static int sh_create(void *thr, const void *battr, void *(*fn)(void *), void *arg) {
  ...
  if (battr) {
    ...
    /* The engine requests lean (128KB) stacks sized for bionic; glibc frames
       are deeper -> overflow -> heap/pointer corruption. Enforce a generous
       minimum (DUCK_STACK_MB, default 8MB) for every game thread. */
    const char *sm = getenv("DUCK_STACK_MB");
    size_t minss = (size_t)(sm && sm[0] ? atoi(sm) : 8) << 20;
    if (ss < minss) ss = minss;
    if (ss >= 32768 && ss <= (256UL << 20)) pthread_attr_setstacksize(&ga, ss);
  } else {
    pthread_attr_setstacksize(&ga, 8UL << 20);
  }
