...
namespace Microsoft.Xna.Framework
{
    internal class SdlGamePlatform : GamePlatform
    {
        ...
        public override void RunLoop()
        {
            ...
            {
                SdlRunLoop();
                CheckQuitCombo();   // PortMaster+: SELECT+START fecha (garantia no binario)
                Game.Tick();
                Threading.Run();
        ...
        // PortMaster+: SELECT(Back)+START fecha o jogo -- GARANTIA NO BINARIO (igual o
        // check_exit_hotkey do Bully/Dysmantle), chamada TODO FRAME. Necessaria porque o
        // gptokeyb faz `gptokeyb "sor4host"` mas a engine .NET renomeia a thread -> o
        // gptokeyb pode NAO achar o processo p/ matar -> o .sh sozinho nao fecha. Aqui
        // lemos o pad direto (o SDL ve o pad mesmo com o gptokeyb, que nao faz grab).
        // 2 vias: (1) pad NATIVO (Back+Start); (2) teclado do gptokeyb (SELECT=Escape,
        // START=Enter). Environment.Exit(0) = _exit imediato -> evita o deadlock do blob
        // Mali ao liberar o contexto GL no teardown normal.
        private void CheckQuitCombo()
        {
            if (_keys.Contains(Keys.Escape) && _keys.Contains(Keys.Enter))
            {
                System.Console.Error.WriteLine("[SOR4] SELECT+START (gptk) -> saindo do jogo");
                System.Environment.Exit(0);
            }
            for (int i = 0; i < 4; i++)
            {
                var gs = GamePad.GetState((PlayerIndex)i);
                if (gs.IsConnected &&
                    gs.Buttons.Back == ButtonState.Pressed &&
                    gs.Buttons.Start == ButtonState.Pressed)
                {
                    System.Console.Error.WriteLine("[SOR4] SELECT+START (pad) -> saindo do jogo");
                    System.Environment.Exit(0);
                }
            }
        }
