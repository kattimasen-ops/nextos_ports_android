...
static void bakeall_step(void *scene) {
  ...
  if (bi < total && TexRead && CreateSprite && Setup) {
    bully_bake_active = 1; bully_bake_cur = bi; bully_bake_total = total; /* p/ a tela de progresso (bake_ui) */
    ...
    const char *sd = getenv("BULLY_PAGE_SWAP");   /* FASE 3: marca o pre-bake do "CD image" (swap name-keyed) pronto */
    if (sd) { char sp[320]; snprintf(sp, sizeof sp, "%s/.swapbake_done", sd); FILE *sf = fopen(sp, "w"); if (sf) fclose(sf); }
