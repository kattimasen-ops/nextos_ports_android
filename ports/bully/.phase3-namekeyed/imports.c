...
/* FASE 3 (pre-bake "CD image"): o swap kind=2 e nomeado pelo NOME da textura (estavel entre runs, igual o
 * etc1cache) -> `<swapdir>/<nome_sanit>_WxH.tx`. Sem nome (textura anonima) -> fallback id-keyed por-run. */
static void bully_swap_key(char *out, size_t n, const char *name, int w, int h) {
  char san[224]; size_t j = 0;
  for (const char *p = name; p && *p && j < sizeof(san) - 1; p++) san[j++] = (*p == '/' || *p == '\\') ? '_' : *p;
  san[j] = '\0';
  snprintf(out, n, "%s/%s_%dx%d.tx", bully_swapdir(), san, w, h);
}
static void bully_swap_path(char *out, size_t n, const char *name, unsigned id, int w, int h) {
  if (name && name[0]) bully_swap_key(out, n, name, w, h);
  else snprintf(out, n, "%s/id_%u.tx", bully_swapdir(), id);   /* anonima: id-keyed por-run */
}
static int bully_swap_ro(void){ static int m=-1; if(m<0)m=getenv("BULLY_SWAP_RO")?1:0; return m; } /* read-only: nao reescreve .tx ja bakeado */
...
static void bully_page_write_swap(unsigned id, int ifmt, int w, int h, unsigned ufmt, unsigned utype, const void *data) {
  ...
  if (g_page_kind[id] != 2) {                  /* 1a vez nesta run: registra (+grava, salvo read-only c/ .tx ja bakeado) */
    const char *nm = bully_cur_tex_path[0] ? bully_cur_tex_path : NULL;
    if (nm && !g_page_name[id]) g_page_name[id] = strdup(nm);   /* name-keyed: estavel entre runs (pre-bake) */
    char path[320]; bully_swap_path(path, sizeof path, g_page_name[id], id, w, h);
    int skip = 0;
    if (bully_swap_ro()) { FILE *e = fopen(path, "rb"); if (e) { fclose(e); skip = 1; } } /* ja bakeado -> NAO reescreve */
    if (!skip) {
...
static int page_req_read(const struct page_req *rq, struct page_ready *out) {
  if (rq->kind == 1) {                                           /* ETC1 (etc1cache por nome) */
    ...
    return 1;
  } else if (rq->kind == 2 && bully_swapdir()) {                 /* SWAP nativo (name-keyed, com cabecalho) */
    char path[320]; bully_swap_path(path, sizeof path, rq->name, rq->id, rq->w, rq->h);
...
static void bully_page_fault(unsigned target, unsigned id) {
  ...
  if (g_page_name[id]) { strncpy(rq.name, g_page_name[id], sizeof rq.name - 1); rq.name[sizeof rq.name - 1] = '\0'; }
  else rq.name[0] = '\0';
  ...
  for (int i = 0; ids && i < n; i++) { unsigned id = ids[i];
    if (id < RESMAP) {
      if (g_page_kind[id]) {   /* PAGINACAO: motor deletou uma pageavel -> limpa estado (id pode ser reusado) */
        ...
        /* FASE 3: o .tx agora e name-keyed (CD image persistente, compartilhado entre runs) -> NAO apagar. */
