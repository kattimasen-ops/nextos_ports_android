...
( sleep 220; for e in /proc/*/exe; do case "$(readlink "$e" 2>/dev/null)" in */lcs) sync; reboot -f;; esac; done ) >/dev/null 2>&1 &
...
export LCS_START=force LCS_STARTFRAME=30 LCS_MAXFRAMES=120 LCS_GLSTATS=1 LCS_INITRESTART=1 LCS_BOUNDSTREAM=1 LCS_STREAMDIAG=1
./lcs >run.log 2>&1
rc=$?
