...
for p in run30.watchdog.pid run30.progress.pid; do
  if [ -r "$p" ]; then
...
: "${LCS_NO_FADEHACK:=0}"   # s6: fade-hacks agora cientes-de-cutscene -> gameplay VISIVEL sem quebrar cutscene
...
export LCS_MAXSECONDS LCS_GLSTATS LCS_BOUNDSTREAM=1 LCS_STREAMER_MAX LCS_NOON=1 LCS_NOON_FRAMES="${LCS_NOON_FRAMES:-300}" LCS_NOPVS=0 LCS_INITONCE=1 LCS_INITLIMIT LCS_RESTARTGUARD=1 LCS_RENDERDIAG LCS_INPUTDIAG LCS_GFX_LOW LCS_GFX_PREFS LCS_SHADOWS_OFF LCS_PVS_CLEAN LCS_ALPHA_DIAG LCS_NO_WORLD_ALPHA LCS_RESOURCECREATOR LCS_RESOURCEDRAIN_MAX LCS_PICKUPGUARD=1 LCS_PICKUPUPDATEGUARD=1
export LCS_CUTSCENE_SPLINEFIX LCS_CUTSCENE_CAMPROCESS LCS_CUTSCENE_CAMPROCESS_STOPPOS LCS_CUTSCENE_FINISH_POS LCS_CUTSCENE_CLEAR_AFTER_FINISH LCS_CUTSCENE_RESTORE_CAMERA LCS_CUTSCENE_POST_RECONCILE LCS_GAMEPLAY_RELEASE_DELAY LCS_SHOT_FINAL_WINDOW
export LCS_CUTSCENE_PAD_SKIP LCS_ANALOG_AS_DPAD LCS_DPAD_AS_AXIS_ONLY LCS_PADBRIDGE_DIRECT LCS_PADBRIDGE_MOVE LCS_PAD_CLEAR_GATE162 LCS_MOVE_RAW LCS_MOVE_AXIS_X LCS_MOVE_AXIS_Y LCS_RAWAXISDIAG LCS_RAW_BUTTONS LCS_BUTTON_RAW_ONLY LCS_ENABLE_BACK_BUTTON LCS_ENABLE_EXIT_HOTKEY LCS_TRIGGER_BUTTONS LCS_TRIGGER_AXES LCS_INPUT_PROBE_ONLY LCS_AXIS_DEADZONE
export LCS_HB_EVERY LCS_SHOT_KEEP_LAST
printf '[run30] maxseconds=%s watchdog=%s streamer_max=%s cutscene=%s pop=%s heli=%s userdisplay=%s start=%s startframe=%s glstats=%s renderdiag=%s inputdiag=%s gfx_low=%s gfx_prefs=%s shadows_off=%s pvs_clean=%s alpha_diag=%s no_world_alpha=%s resource=%s drainmax=%s noon_frames=%s padskip=%s analog_as_dpad=%s dpad_axis_only=%s paddirect=%s padmove=%s padclear162=%s move_raw=%s move_axes=%s,%s rawaxisdiag=%s raw_buttons=%s raw_only=%s back_button=%s exit_hotkey=%s trigger_buttons=%s trigger_axes=%s probe_only=%s axisdz=%s fe25_postready=%s force_gamepad=%s tv=%s njoy=%s shotwin=%s shot_keep_last=%s\n' "$LCS_MAXSECONDS" "$LCS_WATCHDOG_SECONDS" "$LCS_STREAMER_MAX" "$LCS_ENABLE_CUTSCENE" "$LCS_ENABLE_POP" "$LCS_ENABLE_HELI" "$LCS_ENABLE_USERDISPLAY" "${LCS_START:-menu}" "${LCS_STARTFRAME:-}" "$LCS_GLSTATS" "$LCS_RENDERDIAG" "$LCS_INPUTDIAG" "$LCS_GFX_LOW" "$LCS_GFX_PREFS" "$LCS_SHADOWS_OFF" "$LCS_PVS_CLEAN" "$LCS_ALPHA_DIAG" "$LCS_NO_WORLD_ALPHA" "$LCS_RESOURCECREATOR" "$LCS_RESOURCEDRAIN_MAX" "$LCS_NOON_FRAMES" "$LCS_CUTSCENE_PAD_SKIP" "$LCS_ANALOG_AS_DPAD" "$LCS_DPAD_AS_AXIS_ONLY" "$LCS_PADBRIDGE_DIRECT" "$LCS_PADBRIDGE_MOVE" "$LCS_PAD_CLEAR_GATE162" "$LCS_MOVE_RAW" "$LCS_MOVE_AXIS_X" "$LCS_MOVE_AXIS_Y" "$LCS_RAWAXISDIAG" "$LCS_RAW_BUTTONS" "$LCS_BUTTON_RAW_ONLY" "$LCS_ENABLE_BACK_BUTTON" "$LCS_ENABLE_EXIT_HOTKEY" "$LCS_TRIGGER_BUTTONS" "$LCS_TRIGGER_AXES" "$LCS_INPUT_PROBE_ONLY" "$LCS_AXIS_DEADZONE" "${LCS_FE25_POSTREADY:-0}" "${LCS_FORCE_GAMEPAD_UI:-0}" "${LCS_TV_DEVICE:-0}" "${LCS_NJOY_COUNT:-auto}" "$LCS_SHOT_FINAL_WINDOW" "$LCS_SHOT_KEEP_LAST" >run.log
printf '[run30] camprocess=%s camstop=%s finishpos=%s post_reconcile=%s gameplay_release_delay=%s hb_every=%s hb_fsync=%s\n' "$LCS_CUTSCENE_CAMPROCESS" "$LCS_CUTSCENE_CAMPROCESS_STOPPOS" "$LCS_CUTSCENE_FINISH_POS" "$LCS_CUTSCENE_POST_RECONCILE" "$LCS_GAMEPLAY_RELEASE_DELAY" "$LCS_HB_EVERY" "${LCS_HB_FSYNC:-0}" >>run.log
./lcs >>run.log 2>&1
