...
static void crash_print_addr(const char *tag, uintptr_t addr) {
  uintptr_t tb = (uintptr_t)text_base;
  fprintf(stderr, "  %s=%p", tag, (void *)addr);
  if (tb && addr >= tb && addr < tb + text_size) {
    fprintf(stderr, " in libGame+0x%lx\n", (unsigned long)(addr - tb));
    return;
  }
  ...
  if (m) {
    while (fgets(line, sizeof(line), m)) {
      unsigned long a, b;
      char perm[8] = {0}, path[300] = {0};
      if (sscanf(line, "%lx-%lx %7s %*x %*x:%*x %*d %299[^\n]",
                 &a, &b, perm, path) >= 3 &&
          addr >= a && addr < b) {
  ...
  fprintf(stderr, " (unmapped/unknown)\n");
}
...
static void crash_handler(int sig, siginfo_t *info, void *uc) {
  ...
  uintptr_t tb = (uintptr_t)text_base;
  fprintf(stderr, "\n=== CRASH sig=%d code=%d addr=%p pid=%d uid=%d ===\n",
          sig, info->si_code, info->si_addr, info->si_pid, info->si_uid);
  ...
  crash_print_addr("PC", pc);
  crash_print_addr("LR", lr);
  fprintf(stderr, "  backtrace (fp chain):\n");
  ...
  for (int i = 0; i < 32 && fp; i++) {
    ...
    if (!rlr) break;
    char tag[16];
    snprintf(tag, sizeof(tag), "#%-2d lr", i);
    crash_print_addr(tag, rlr);
    if (nfp <= fp) break;
    fp = nfp;
  }
  /* identifica a lib + offset do PC e do registrador-arg (x0..x2) via /proc/self/maps */
  fprintf(stderr, "  regs: x0=%p x1=%p x2=%p x3=%p\n",
          (void*)u->uc_mcontext.regs[0], (void*)u->uc_mcontext.regs[1],
          (void*)u->uc_mcontext.regs[2], (void*)u->uc_mcontext.regs[3]);
  crash_print_addr("x0", u->uc_mcontext.regs[0]);
  crash_print_addr("x1", u->uc_mcontext.regs[1]);
  crash_print_addr("x2", u->uc_mcontext.regs[2]);
#endif
  fflush(stderr);
  _exit(128 + sig);
}
...
int main(int argc, char *argv[]) {
  (void)argc; (void)argv;
  setvbuf(stderr, NULL, _IONBF, 0); /* log completo mesmo em crash/hang (sem buffer) */
  { volatile char c = g_bionic_guard_pad[0]; (void)c; } /* ancora o TLS pad (nunca escreve) */
  if (!getenv("LCS_NOCRASHHANDLER")) install_crash_handler(); /* pular p/ gdb pegar o SIGSEGV */
